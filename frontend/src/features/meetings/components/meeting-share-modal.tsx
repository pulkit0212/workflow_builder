...
const INTEGRATION_META: Record<IntegrationType, { label: string; icon: string; what: string }> = {
  slack:  { label: "Slack",  icon: "💬", what: "Summary + action items to channel" },
  gmail:  { label: "Gmail",  icon: "📧", what: "Summary email to recipients" },
  notion: { label: "Notion", icon: "📝", what: "Full page with summary + transcript" },
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900">Share Meeting Summary</p>
              <p className="text-xs text-slate-400 truncate max-w-[220px]">{meeting.title}</p>
            </div>
          </div>
            ...
            <>
              {allDone && (
                <div className={cn(
                  ...
                  successCount === totalCount ? "bg-emerald-50 text-emerald-700"
                    : successCount === 0 ? "bg-red-50 text-red-600"
                    : "bg-amber-50 text-amber-700"
                )}>
                  {successCount === totalCount
                    ? <><CheckCircle2 className="h-4 w-4" /> Shared to all {totalCount} destination{totalCount !== 1 ? "s" : ""}</>
