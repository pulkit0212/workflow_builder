...
function getPlatformBadge(platform: string) {
  switch (platform) {
    case "zoom": return { label: "Zoom", cls: "bg-blue-50 text-blue-600 ring-blue-200" };
    case "teams": return { label: "Microsoft Teams", cls: "bg-[#f5f3ff] text-[#6264A7] ring-[#ddd6fe]" };
    default: return { label: "Google Meet", cls: "bg-emerald-50 text-emerald-700 ring-emerald-200" };
  }
}
...
type CalendarMeetingRowProps = {
  ...
  adminWorkspaces?: WorkspaceRecord[];
};
...
export function CalendarMeetingRow({ meeting, session, adminWorkspaces = [] }: CalendarMeetingRowProps) {
  ...
  // A meeting is already shared if its linked session has a workspaceId set
    ...
    try {
      const res = await fetch("/api/meetings/share-calendar", {
        ...
        body: JSON.stringify({
          ...
          externalCalendarEventId: meeting.id, // Google Calendar event ID for matching
        }),
      });
  ...
  return (
    <div className="group relative flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-[#6c63ff]/40 hover:bg-[#faf9ff] hover:shadow-md hover:shadow-[#6c63ff]/10 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-shrink-0 flex-wrap items-center gap-2">
        ...
        {/* Share button — only for admins, only if not already shared */}
