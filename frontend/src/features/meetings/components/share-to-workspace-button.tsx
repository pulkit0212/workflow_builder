...
import { ArrowRight, CheckCircle, Clock, Loader2, Share2, X } from "lucide-react";
...
export function ShareToWorkspaceButton({
  ...
  const [allWorkspaces, setAllWorkspaces] = useState<WorkspaceRecord[] | null>(null);
  ...
  useEffect(() => {
    ...
    apiFetch("/api/workspaces", { cache: "no-store" })
      .then((res) => res.json())
      .then((data: { success: boolean; workspaces: WorkspaceRecord[] } | WorkspaceRecord[]) => {
        const list = Array.isArray(data)
          ? data
          : ((data as { success: boolean; workspaces: WorkspaceRecord[] }).workspaces ?? []);
        setAllWorkspaces(list);
        if ((moveStatus === "approved" || moveStatus === "pending") && workspaceId) {
  ...
  }, [isOwner, moveStatus, workspaceId, apiFetch]);
    ...
    startTransition(async () => {
      try {
        ...
        if (isCalendar && calendarMeeting) {
          res = await apiFetch("/api/meetings/share-calendar", {
            method: "POST",
            body: JSON.stringify({
          ...
          res = await apiFetch(`/api/meetings/${meetingId}/move-to-workspace`, {
            method: "POST",
        ...
        const data = await res.json() as {
          ...
          details?: { error?: string }; meetingId?: string;
        };
        ...
        if (isCalendar && data.meetingId) setDbMeetingId(data.meetingId);

        const shared = allWorkspaces?.find((w) => w.id === selectedWorkspaceId);
        setWorkspaceName(shared?.name ?? null);
        setWorkspaceId(selectedWorkspaceId);
        setMoveStatus(data.status ?? "approved");
        setModalOpen(false);

        const msg = data.status === "pending"
          ? `Request sent to "${shared?.name ?? "workspace"}" admin for approval.`
          : `Shared to "${shared?.name ?? "workspace"}" successfully.`;
  ...
  function handleRemove() {
    ...
    startTransition(async () => {
      try {
        const res = await apiFetch(`/api/meetings/${idToUse}/move-to-workspace`, { method: "DELETE" });
      ...
      <>
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1.5 text-sm font-medium text-amber-700 ring-1 ring-amber-200">
            ...
            Pending approval{workspaceName ? ` — ${workspaceName}` : ""}
          </span>
          {isOwner && (
            <button
              type="button"
  ...
  // Still loading workspaces — don't permanently hide the button
  if (isLoading) return null;

  // Not the owner — never show the share button
  if (!isOwner) return Toast ?? null;

  // No workspaces to share to
  if (!allWorkspaces || allWorkspaces.length === 0) return Toast ?? null;

  const workspacesToShow = allWorkspaces;
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-400">
                  Admins share instantly. Members need admin approval.
                </p>
              </div>
            </div>
              ...
              >
                {allWorkspaces?.map((ws) => (
                  <option key={ws.id} value={ws.id}>
                    {ws.name}{ws.role === "admin" ? " (Admin)" : " (Member)"}
                  </option>
                ))}
              </select>
              {selectedWorkspaceId && workspacesToShow.find((w) => w.id === selectedWorkspaceId)?.role !== "admin" && (
                <p className="text-xs text-amber-600">⚠ A workspace admin will need to approve this request.</p>
              )}
            </div>
