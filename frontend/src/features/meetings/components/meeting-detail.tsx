...
// ── CalendarEventDetail — shown when clicking a Google/Microsoft calendar event ──
...
export function CalendarEventDetail({ encodedId }: { encodedId: string }) {
  ...
  useEffect(() => {
    // Re-fetch the calendar feed to find this specific event
    void (async () => {
      try {
        const now = new Date();
        ...
        const res = await clientApiFetch(`/api/meetings/calendar-feed?${params.toString()}`, { cache: "no-store" });
        if (!res.ok) return;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-24">
        <LoaderCircle className="h-7 w-7 animate-spin text-[#6c63ff]" />
      </div>
    );
  }
  ...
  if (!event) {
    return (
      <div className="space-y-4">
        <button type="button" onClick={() => router.back()} className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-500 hover:text-slate-700">
          <ArrowLeft className="h-4 w-4" /> Back to meetings
        </button>
        <Card className="p-8 text-center">
          <AlertTriangle className="mx-auto h-8 w-8 text-amber-400" />
          <p className="mt-3 text-sm font-semibold text-slate-700">Calendar event not found</p>
          <p className="mt-1 text-xs text-slate-400">This event may have been deleted or is outside the visible date range.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-[#6c63ff]">{providerLabel}</p>
        ...
        {event.meetLink && (
          <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 flex items-center justify-between gap-4">
            <div className="min-w-0">
              <p className="text-xs font-semibold text-slate-400">Meeting Link</p>
              <p className="mt-0.5 truncate text-sm text-[#6c63ff]">{event.meetLink}</p>
            </div>
            <a
              href={event.meetLink}
              target="_blank"
              rel="noopener noreferrer"
              className="shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-[#6c63ff] px-4 py-2 text-sm font-semibold text-white hover:bg-[#5b52e0] transition-colors"
            >
              Join Meeting
            </a>
          </div>
        ...
        <div className="rounded-xl border border-[#6c63ff]/20 bg-[#faf9ff] px-4 py-3">
...
export function MeetingDetail({ meetingId }: MeetingDetailProps) {
      ...
      void fetchMeetingById(meetingId)
        .then((nextMeeting) => {
          ...
          if (!nextMeeting.insights && session.state === "completed") {
            ...
            const retryInsights = () => {
              retries++;
              if (retries > 15) return; // retry for up to ~2 minutes
              setTimeout(() => {
                void fetchMeetingById(meetingId)
                  .then((refreshed) => {
                  ...
                  .catch(() => null);
              }, retries * 2000); // 2s, 4s, 6s... up to 30s intervals
            };
            retryInsights();
          }
        })
  ...
  function handleMoveToWorkspace() {
    ...
    startMoveTransition(async () => {
      try {
        const res = await clientApiFetch(`/api/meetings/${meetingId}/move-to-workspace`, {
          method: "POST",
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      if (!res.ok) {
        const data = (await res.json()) as { message?: string };
        setDeleteError(data.message ?? "Failed to delete meeting.");
        setIsDeleting(false);
        return;
      }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[14px]">
                The following integration{session.autoShareFailures.length > 1 ? "s" : ""} failed to receive the meeting summary:{" "}
                <strong>{session.autoShareFailures.map((f) => f.integration).join(", ")}</strong>.
                Check your integration settings and try sharing manually.
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => setAutoShareFailuresDismissed(true)}
            className="shrink-0 rounded-lg px-3 py-1.5 text-sm font-medium text-[#991b1b] hover:bg-[#fecaca] transition-colors"
            aria-label="Dismiss auto-share failure banner"
          >
            Dismiss
          </button>
        </div>
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-[#e5e7eb] bg-white p-2">        {tabs.map((tab) => (
          <button
            key={tab.id}
            type="button"
        ...
        >
          <div className="mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 id="delete-dialog-title" className="text-base font-semibold text-[#111827]">
                  Delete meeting?
                </h2>
                <p className="mt-1 text-sm text-[#6b7280]">
                  This will permanently delete <strong>{meeting.title}</strong> and all associated data. This action is irreversible.
                </p>
              </div>
            </div>
            {deleteError ? (
