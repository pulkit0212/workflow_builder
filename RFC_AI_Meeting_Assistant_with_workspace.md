...
## 28. Workspace Support (Team Collaboration Layer)

### Overview

In addition to individual meetings, the platform will support **Workspaces** that allow teams to collaborate, share meetings, and manage meeting insights together.

This feature does **not change the current individual meeting functionality**. Users can continue recording and managing personal meetings exactly as before.

The system simply introduces an additional context where meetings may optionally belong to a workspace.
...
### Meeting Table Update

The existing `meeting` table will be extended with an optional `workspace_id`.
...
- `workspace_id = NULL` → personal meeting
- `workspace_id != NULL` → workspace meeting

This allows the same meeting pipeline to support both contexts without changing existing services.
...
### Optional Feature: Move Personal Meeting to Workspace

Users may convert a personal meeting into a workspace meeting.
...
This enables users to record privately first and later share with their team.
...
Selecting a workspace will show all meetings belonging to that workspace.
...
Workspace support only affects:

- meeting ownership
- visibility
- authorization

All downstream AI processing remains unchanged.

---

### Summary

The workspace feature introduces a collaboration layer without modifying the existing individual meeting architecture. Meetings can exist either independently or inside a workspace, allowing the platform to support both personal and team workflows seamlessly.
